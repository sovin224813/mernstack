import React, { useState } from "react";
import API from "../utils/api";
import { useNavigate, Link } from "react-router-dom";

export default function Register() {
  const [username, setUsername] = useState('');
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [errors, setErrors] = useState({});
  const navigate = useNavigate();

  const handleSubmit = async (e) => {
    e.preventDefault();
    setErrors({});
    try {
      await API.post("/auth/register", { username, email, password });
      navigate("/login");
    } catch (err) {
      console.error("Registration failed:", err);
      if (err.response && err.response.data) {
        const data = err.response.data;
        if (data.errors) {
          const validationErrors = {};
          data.errors.forEach(error => {
            validationErrors[error.path] = error.msg;
          });
          setErrors(validationErrors);
        } else if (data.message) {
          setErrors({ general: data.message });
        } else {
          setErrors({ general: "An unknown server error occurred." });
        }
      } else if (err.request) {
        setErrors({ general: "Cannot connect to the server. Please check your connection." });
      } else {
        setErrors({ general: err.message || "An unknown error occurred." });
      }
    }
  };

  return (
    <div className="flex items-center justify-center min-h-screen bg-[var(--color-bg-secondary)] px-4">
      <div className="w-full max-w-md">
        <form onSubmit={handleSubmit} className="bg-[var(--color-card)] p-8 rounded-lg shadow-xl">
          <h2 className="text-3xl font-bold mb-6 text-center text-[var(--color-text)]">
            Create an Account
          </h2>

          {errors.general && (
            <p className="text-[var(--color-danger)] text-sm mb-4 text-center">{errors.general}</p>
          )}

          <div className="mb-4">
            <label htmlFor="username" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">
              Username
            </label>
            <input
              id="username"
              className={`block w-full border bg-[var(--color-input)] text-[var(--color-text)] p-3 rounded-md shadow-sm focus:outline-none focus:ring-2 ${errors.username ? 'border-red-500 focus:ring-red-500' : 'border-[var(--color-input-border)] focus:ring-[var(--color-brand)]'}`}
              type="text"
              value={username}
              onChange={e => setUsername(e.target.value)}
              placeholder="yourusername"
            />
            {errors.username && <p className="text-[var(--color-danger)] text-xs mt-1">{errors.username}</p>}
          </div>

          <div className="mb-4">
            <label htmlFor="email" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">
              Email
            </label>
            <input
              id="email"
              className={`block w-full border bg-[var(--color-input)] text-[var(--color-text)] p-3 rounded-md shadow-sm focus:outline-none focus:ring-2 ${errors.email ? 'border-red-500 focus:ring-red-500' : 'border-[var(--color-input-border)] focus:ring-[var(--color-brand)]'}`}
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="you@example.com"
            />
            {errors.email && <p className="text-[var(--color-danger)] text-xs mt-1">{errors.email}</p>}
          </div>

          <div className="mb-6">
            <label htmlFor="password" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">
              Password
            </label>
            <div className="relative">
              <input
                id="password"
                className={`block w-full border bg-[var(--color-input)] text-[var(--color-text)] p-3 rounded-md shadow-sm focus:outline-none focus:ring-2 pr-16 ${errors.password ? 'border-red-500 focus:ring-red-500' : 'border-[var(--color-input-border)] focus:ring-[var(--color-brand)]'}`}
                type={showPassword ? "text" : "password"}
                value={password}
                onChange={e => setPassword(e.target.value)}
                placeholder="••••••••"
              />
              <button
                type="button"
                className="absolute inset-y-0 right-0 flex items-center px-4 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-brand)]"
                onClick={() => setShowPassword(!showPassword)}
              >
                {showPassword ? "Hide" : "Show"}
              </button>
            </div>
            {errors.password && <p className="text-[var(--color-danger)] text-xs mt-1">{errors.password}</p>}
          </div>

          <button
            className="w-full bg-[var(--color-brand)] text-white p-3 rounded-md font-semibold hover:bg-[var(--color-brand-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--color-brand)] focus:ring-offset-2 transition duration-200"
            type="submit"
          >
            Register
          </button>

          <div className="mt-6 text-center">
            <Link to="/login" className="text-sm text-[var(--color-brand)] hover:underline">
              Already have an account? Login
            </Link>
          </div>
        </form>
      </div>
    </div>
  );
}